            ...
            document.getElementById("category-trending-sung-container").insertAdjacentHTML(
                ...
                `
                       <div class="swiper-slide">
                          <a href="/albums/details/${music.album}">
...
;(async () => {
    try {
        const response = await fetch(`/api/v1/musics/?page=1&limit=20&status=latest&genre=${location.href.split('/').pop()}`)
        const data = await response.json()

        data.data.forEach(music => {
            document.getElementById("latest-container").insertAdjacentHTML(
                "beforeend",
                `
                       <div class="swiper-slide">
                          <a href="/albums/details/${music.album}">
